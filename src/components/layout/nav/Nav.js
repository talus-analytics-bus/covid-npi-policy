...
import logo from "../../../assets/images/logo-talus.png";
...
const Nav = props => {
  ...
  const renderCountryPicker = regionName => {
    ...
    return props.places
      .find(p => p.name === regionName)
      .data.map(c => (
        <Link
          onClick={e => countryOnClick(e, c)}
          className={classNames(styles.country, {
            [styles.active]: c[0] === curCountry
          })}
          to={`/details/${c[0]}`}
        >
          {c[1]}
  ...
  function regionOnClick(e) {
    ...
    if (el.classList.contains(styles.activeRegion)) {
      ...
      // TODO use flags instead of direct DOM manipulation
  ...
  const renderLocationPicker = () => {
    if (props.places.length === 0)
      return (
        <div
          className={classNames(styles.locationPicker, {
            [styles.visible]: showLocationPicker
          })}
        />
      );
    return (
      ...
      >
        <div className={styles.header}>Countries by region</div>
        <div className={styles.content}>
          ...
          {searchResults !== null &&
            searchResults.map(c => (
              <Link
                onClick={e => searchedCountryOnClick(e, c)}
                className={classNames(styles.countryResult, {
                  [styles.active]: c.id === curCountry
                })}
                to={`/details/${c.id}`}
              >
                {c.name}
      ...
      return (
        <Link
          ...
          className={classNames(
            page === button.page ? styles.active : "",
            styles.navButtonContainer
          )}
        >
          <div className={classNames(styles.buttonSpinner)} />
          <div
  ...
  const getPageSubtitle = page => {
    ...
    switch (page) {
      case "map":
        return (
          "Showing most recent data as of " +
          Util.today().toLocaleString("en-US", {
            month: "short",
            year: "numeric",
            day: "numeric",
            hour: "numeric",
    ...
    <div className={styles.navWrapper}>
      ...
      >
        ...
        <div className={styles.text}>
          <div id={"navTitle"} className={styles.title}>
            {getPageTitle(page)}
          </div>
        ...
        <ReactTooltip
          ...
          effect="float"
          delayShow={500}
          getContent={message => <div>{message}</div>}
        />
      </div>
    </div>
  );
